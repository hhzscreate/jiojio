<template>
	<view class="container">
		...
		<view class="tabbar">
			<view class="item" :class="{active: !tabIndex}" @tap="switchTab(0)">所有订单</view>
				...
				<view class="history-order">
					<view class="menu">
						<view class="flex-fill d-flex justify-content-start">
							...
							<view class="item" :class="{active: orderMenuIndex}" @tap="switchMenuTab(1)">其他订单</view>
						</view>
					...
					<swiper :current="orderMenuIndex" :duration="300" :show-scrollbar="false" class="history-order-swiper">
							...
							<scroll-view scroll-y="true" class="orders-scroll">
								...
								<view class="wrapper">
									...
									<view class="order-list">
										...
										<!-- 测试怎么传递参数 -->
										<navigator class="order" v-for="(order, index) in orders" :key="index" open-type="navigate" :url="'/pages/order/detail?order=' + encodeURIComponent(JSON.stringify(order))">
											...
											<view class="action">
												...
												<button type="primary" plain hover-class="none" @tap.stop="tiaozhuan">再来一单</button>
											</view>
										</navigator>
									</view>
								</view>
						...
						<!-- 百货订单 begin -->
						<swiper-item @touchmove.stop="handleSwiperItemChange" >
							<view class="store-order-wrapper" v-show="true">
								<image src="/static/images/order/default_img_order.png"></image>
								<view>您还没有下单</view>
								<view>快去商城逛逛吧</view>
							</view>
