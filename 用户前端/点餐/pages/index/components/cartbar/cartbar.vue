...
<template>
	<view>
		<uni-transition :mode-class="['slide-bottom']" :show="!!cartNum" :styles="cartBarStyles">
			...
			<!-- 向上调用，哪里调用了carbar去找那个pay方法 -->
			<button type="primary" class="right" @tap="pay">结算</button>  
		</uni-transition>
		
		<!-- ref是标签，用于关闭的时候找到他 -->
		<!-- :cart="cart"即表示cartbar里面的cart会传递给cart-popup -->
...
<script>
import uniTransition from '@/components/uni-transition/uni-transition.vue'  //这个是carbar的父组件
...
export default {
	...
	methods: {
		...
		clear() {
			this.$emit('clear')//意思是去父组件，父组件就是用到了carbar的那个地方，里面去执行这个函数
		},
		pay() {
			this.$emit('pay')
		}
