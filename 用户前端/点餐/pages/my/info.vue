<template>
	<view class="container">
		...
		<view class="save-btn">
			<button type="info" @tap="setuserInfo">保存</button>
		</view>
		
		<view class="save-btn">
			<button type="info" @tap="quit" :disabled="is_Login">退出账号</button>
		</view>
		...
		<modal :show="telphoneModalVisible" custom padding="40rpx 30rpx">
			<view class="telphone-modal">
				...
				<view class="tips">
					为保障您正常使用剁jiojio小程序，请在绑定前确定新手机号未绑定剁jiojio。
				</view>
...
<script>
	...
	export default {
		...
		//请求加载数据
		//会传入参数user_id来进行选择用哪条数据初始化信息页面
		//成功！
		async onLoad({user_id}) {
			if(user_id) {
				const res=await this.$myRequest({
					url:'/getuserInfo/',
					data:{
						userId:user_id,//应该是userId:user_id，先用静态数据测试
						//id:id
					}
				})
		...
		methods: {
			...
			async setuserInfo(){//保存数据并跳转
				...
				const res=await this.$myRequest({
					url:'/setuserInfo/',
					data:{
						"userId":getApp().globalData.openId,//用来找数据
					    "username":this.form.username,
					    "telphone": this.form.telphone,
					    "gender": this.form.gender,
					    "birthday": this.form.birthday,
					    //"url": userinfo.url   图片更改暂时不能
					        }
				})
				// console.log(res.data.data)
				//console.log(this.form.gender)
				//上面其实同样更改了数据库里的userName，我们在这里该global即可
				getApp().globalData.userName=this.form.username
